...
The correlation store's scope is entered once, in an `onRequest` extension (`src/api/common/helpers/correlation-scope.js`), registered ahead of the request logger. The route handlers only set the value once it is known; they no longer open their own scope around the handler body. This is why the `[response]` line the request logger writes after the handler has returned still carries `transaction.id` — a scope opened only around the handler would have already closed by then. Do not reintroduce a handler-local scope: it would put the id back on the handler's own log lines but silently drop it from the response line.
